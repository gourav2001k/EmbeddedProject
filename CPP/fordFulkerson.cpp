...
bool DFS(vector<vector<int>> &graph, vector<int> &parent, vector<bool> &visited, int n, int node, int t) {
	if (node == t) return true;
	visited[node] = true;

	for(int u = 0; u < n; u++) {
		if (graph[node][u] > 0 && !visited[u]) {
			parent[u] = node;
			if (DFS(graph, parent, visited, n, u, t))
				return true;
		}
	}
	return false;
...
int FordFulkerson(vector<vector<pair<int, int>>> &adj, int n, int s, int t) {
...
int main(){
	...
	int max_flow = FordFulkerson(adj, n, s, t);
